...
function imageDuplicateKey(item) {
  const b = rectBoundsFromItem(item);
  const hrefKey = String(item.href || "").slice(0, 120);
  return [
    hrefKey,
    Math.round(b.x),
    Math.round(b.y),
...
async function createImageRect(item, parsed, namePrefix) {
  ...
  rect.name =
    ...
    " " +
    item.index +
    (item.clipApplied ? " clipped " + item.clipPathId : "");
...
async function importImages(images) {
  ...
  for (const item of images) {
    const key = imageDuplicateKey(item);
...
function buildExternalAssignmentReport(imageSummary) {
  ...
  for (let i = 0; i < assignments.length; i++) {
    ...
    report +=
      ...
      "  Method: " + a.method + "\n" +
      "  Clip applied: " + (a.clipApplied ? "yes" : "no") + "\n" +
      "  Clip path ID: " + (a.clipPathId || "") + "\n" +
      "  Clip error: " + (a.clipError || "") + "\n" +
      "  SVG URL: " + a.svgHref + "\n" +
...
penpot.ui.onMessage(async (message) => {
  ...
  try {
    ...
    const imageResult = await importImages(images);
    ...
    const structure = imageSummary.structureDiagnostics || {};
    ...
    sendStatus(
      ...
      "Guided slot files provided: " + (imageSummary.externalSlotFilesProvided ?? "unknown") + "\n" +
      ...
      "Matched by guided slot: " + (imageSummary.externalMatchedBySlot ?? "unknown") + "\n" +
      "Matched by filename/path: " + (imageSummary.externalMatchedByName ?? "unknown") + "\n" +
      "Matched by Miro resource ID: " + (imageSummary.externalMatchedByResourceId ?? "unknown") + "\n" +
      "Matched by batch order: " + (imageSummary.externalMatchedByOrder ?? "unknown") + "\n" +
      "Batch-order matching disabled: " + (imageSummary.externalOrderMatchDisabled ?? "unknown") + "\n" +
      ...
      "Raster images actually created: " + imageResult.created + "\n" +
      "Duplicate raster images skipped: " + imageResult.duplicateSkipped + "\n\n" +

      "SAFE5 CLIP-PATH CROP\n" +
      "safe5 clip-path crop enabled: " + (imageSummary.safe5ClipPathCropEnabled ?? "unknown") + "\n" +
      "direct clip-path images found: " + (imageSummary.directClipPathImagesFound ?? "unknown") + "\n" +
      "direct clip-path images cropped: " + (imageSummary.directClipPathImagesCropped ?? "unknown") + "\n" +
      "direct clip-path crop failed: " + (imageSummary.directClipPathCropFailed ?? "unknown") + "\n\n" +

      "STRUCTURE SUMMARY\n" +
      "clipPath elements: " + (structure.clipPathNodes ?? "unknown") + "\n" +
      "mask elements: " + (structure.maskNodes ?? "unknown") + "\n" +
      "use elements: " + (structure.useNodes ?? "unknown") + "\n" +
      "images with direct clip-path: " + (structure.imagesWithDirectClipPath ?? "unknown") + "\n" +
      "embedded SVG image objects: " + (structure.embeddedSvgImageObjects ?? "unknown") + "\n\n" +
      ...
      "safe5 keeps v0.9.2-style importing and attempts to crop raster images that have direct clip-path attributes."
    );
...
// Local Miro SVG Importer plugin.js version: v0.9.2-safe5